...
@implementation NoiseReduction

+(void)noiseReduceFrom:(NSString *)inputPath to:(nullable NSString *)outputPath{
    
    const char *in_file  = [inputPath  UTF8String];
    const char *out_file = [outputPath UTF8String];
...
+ (void)noise_suppression:(char *)in_file and:(char *)out_file {
    ...
    if (inBuffer != nullptr) {
        ...
        [self nsProcess:inBuffer :sampleRate :(int)inSampleCount :kVeryHigh];
        double time_interval = calcElapsed(startTime, now());
        printf("降噪成功:time interval: %d ms\n ", (int) (time_interval * 1000));
...
+ (int)nsProcess:(int16_t *)buffer :(uint32_t)sampleRate :(int)samplesCount :(enum nsLevel)level {
    ...
    size_t samples = TOOL_MIN(160, sampleRate / 100);
